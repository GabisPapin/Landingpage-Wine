          ...
          modality.map((plan) => (
            <div className="cardMod-container" key={ plan.name }>
              ...
              <img
                className="carimbo"
                src={ carimboDesconto }
                alt="Imagem de um carimbo"
              />
              <h3
                className="plan-price-disc"
              >
                { `R$ ${plan.priceWithDiscount.toFixed(2)}` }
              </h3>
              <img
                className="icon"
                src={ discontIcon }
                alt="Icone de uma etiqueta de desconto"
              />
              <h2 className="promotion">{ plan.promotion }</h2>
              { plan.gift && <img className="gift-icon" src={ gift } alt="Um presente" />}
